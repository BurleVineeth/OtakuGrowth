...
const SkillDetail = () => {
  ...
  const [celebrate, setCelebrate] = useState<"simple" | "grand">();
    ...
    try {
      ...
      setScheduledTasks((prev) => {
        ...
        if (celebrateTimer.current) {
          ...
          setCelebrate(undefined);
        }

        setTimeout(() => {
          setCelebrate(allTasksCompleted ? "grand" : "simple");
        }, 0);
    ...
    <>
      ...
      {celebrate &&
        (celebrate === "simple" ? (
          ...
          <GrandCelebrate message="Congratulations! All tasks completed" />
        ))}
    </>
  );
};
